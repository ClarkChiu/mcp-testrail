 ...
 */
export function registerPlanTools(server, testRailClient) {
    // Get all test plans for a project
    server.tool("getPlans", "Retrieves all test plans for a specified TestRail project / 指定されたTestRailプロジェクトの全テストプランを取得します", getPlansSchema, async ({ projectId }) => {
        try {
            const plans = await testRailClient.plans.getPlans(projectId);
            const successResponse = createSuccessResponse("Test plans retrieved successfully", {
                plans,
            });
            return {
            ...
            const errorResponse = createErrorResponse(`Error fetching test plans for project ${projectId}`, error);
            return {
