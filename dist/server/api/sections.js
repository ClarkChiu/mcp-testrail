...
export function registerSectionTools(server, testRailClient) {
    ...
    server.tool("getSections", "Retrieves sections for a specified project and suite. Supports pagination via limit and offset parameters (default: limit=250, offset=0). Use _links.next to determine if more pages are available. / 指定されたプロジェクトとスイートのセクションを取得します。limitとoffsetパラメータでページネーションをサポートします。", getSectionsSchema, async ({ projectId, suiteId, limit, offset }) => {
        try {
            const params = {};
            if (limit !== undefined)
                params.limit = limit;
            ...
            const result = await testRailClient.sections.getSections(projectId, suiteId, params);
